...
# Executive Summary
This 'screenshot as a service' application lets a user input a list of one or many URLs to websites of interest.  The logic portion of the application uses a third party gem to generate an image file of a screenshot of the webpages that are located at the URLs.  The service is implemented one 'request' at a time, where the request receives the list of URLs and a request name.  The resulting images and URLs of a given request can be recalled for inspection at any time after the request has been completed.

### Developed by Max Aubain
...
1. The latest version of Ruby 2.6.3 and Ruby on Rails 5.2.3 are used here as defined in the `Gemfile`.
```
# install Ruby
$ gem install ruby
...
```
# complete installation of the gems and instantiation of the database
$ bundle install
...
The following sections are my personal notes from while I was building the project.  They are not necessary to understand the application but they are useful in understanding my approach.
